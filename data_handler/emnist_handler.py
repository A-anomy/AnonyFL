...
class Handler(BaseHandler):
    ...
    def load_data(self, load_test = False):
        if not os.path.exists(self.cfg["base_path"] + "/data/emnist"):
            os.makedirs(self.cfg["base_path"] + "/data/emnist")
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5,), (0.5,))] ) # 标准化]
        data_train = datasets.EMNIST(root = self.cfg["base_path"] + "/data/emnist",
                                    transform=transform,
                                    train = True, split = 'letters',
                                    download = True)
        # data_test = datasets.CIFAR10(root = self.cfg["base_path"] + "/data/emnist",
        #                         #    transform = transform,
        ...
        if load_test == True:
            data_train = datasets.EMNIST(root = self.cfg["base_path"] + "/data/emnist",
                                   transform = transform, split = 'letters' ,
                                train = False, download = True)
            data_loader = torch.utils.data.DataLoader(data_train, batch_size=len(data_train.data), shuffle = False)
            for data in data_loader:
                data_train.data, data_train.targets = data
    ...
    def save_data_to_folders(self, data_splits, num_clients):
        save_dir = self.cfg["base_path"] + '/data/emnist/client_data/{}'.format(str(num_clients))
    ...
    def load_data_from_folders(self, client_id, num_clients):
        data_dir = os.path.join(self.cfg["base_path"] + '/data/emnist/client_data/{}/client_{}'.format(str(num_clients), str(client_id)))
...
class DataLoader:
    def __init__(self, cfg, dev_id, batch_size = None):
        self.cfg = cfg
        self.data_dir = os.path.join(cfg["base_path"]+r"/data/emnist/client_data/{}".format(str(cfg["num_clients"])),"client_{}".format(str(dev_id)))
...
class TestLoader:
    ...
    def load_data(self, distribute = False, client_id = 0):
        if distribute:
            data_dir = os.path.join(self.cfg["base_path"]+r"/data/emnist/client_data/{}".format(str(self.cfg["num_clients"])),"client_{}".format(str(client_id)))
            images_file = os.path.join(data_dir, 'images.npy')
            labels_file = os.path.join(data_dir, 'labels.npy')
            images = np.load(images_file)
            labels = np.load(labels_file)
            ...
            data_test = datasets.EMNIST(root = self.cfg["base_path"] + "/data/emnist",
                                    transform = transform,split = 'letters' ,
                                    train = False, download = True)
            test_loader = torch.utils.data.DataLoader(data_test, batch_size=len(data_test.data), shuffle = False)
